...
#' @param function which pci function: qd_pci1 or 
...
#'
#' @param pci1 first df with pci_obs & pci_sd
#' @param pci2 second df with pci_obs & pci_sd
#'
#' @return Vector of statistics
#' @return a df with distance & p - values
#' @export
#'
#' @examples
#' \dontrun{
#' res <- qd_pci_2sample_test(pci1 = "sim2023_WBA", pci2 = "sim2020_WBA")
#' }
qd_pci_2sample_test <- function(pci1=NULL, pci2=NULL) {
  
  if (is.null(pci1) | is.null(pci2)){
    stop("pci1, pci2 must be provided")
  }
  
  pci1_df <- get(pci1)
  pci2_df <- get(pci2)
  
  pci1_obs <- pci1_df$pci_obs
  pci1_sd <- pci1_df$pci_sd
  
  pci2_obs <- pci2_df$pci_obs
  pci2_sd <- pci2_df$pci_sd
  
  d <- (pci1_obs - pci2_obs) / sqrt(pci1_sd^2 + pci2_sd^2) #geen absolute waarde gebruiken zoals in voorbeeld excel, want dat is fout
...
bootstrap_compare_pci2 <- function(x, m = 1, p = 1, 
  ...
  dfPCI <- NULL
  dfCheck <- expand.grid(g1 = unique(dfExpanded$group), g2 = unique(dfExpanded$group), 
                         smaller = 0, greater = 0) %>% filter(g1 < g2)
  for (i in 1:nsamp){
    dfUsed <- 
      dfExpanded %>%
      slice(sample(1:nrow(dfExpanded), 
                   ...
                   replace = TRUE)) %>%
      group_by(group) %>% 
      summarize(pci = qd_pci2(score, 
                              scale_values = scale_values,
                              p = p, 
                              m = m), 
                iter = i)
    for (k in 1:nrow(dfCheck)) {
      g1 <- filter(dfUsed, group == dfCheck[k,"g1"]) %>% pull(pci)
      g2 <- filter(dfUsed, group == dfCheck[k, "g2"]) %>% pull(pci)
  ...
  dfCheck <- dfCheck %>% mutate(fractieSmaller = smaller / nsamp)
  list(dfPCI, dfCheck)
}
